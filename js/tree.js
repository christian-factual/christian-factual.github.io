...
function update(source, root, tree, svg, diagonal) {
  ...
  nodeEnter.append("text")
      ...
      .text(function(d) {
      return d.name[$('#dropdown').val()]; //this should be the language
    })
      .style("fill-opacity", 1e-6);
...
function calculateRadius(d, country){
  //d is the node element
  //country code that we're resizing to
  var largest = _.max(_.toArray(countriesCount[country]), function(val){return val})
  var scale = d3.scale.log()
                .domain([1, largest])
                .range([1, 5]);
  countriesCount[country]//this gets a object of the counts; key: "id" & value: #count
  if(d.category_id !== "1"){
    var count = countriesCount[country][d.category_id]+1 //this is the number of counts
    return scale(count);
  }
}

function changeCountryCounts(country){
  console.log("calling resize!", countriesCount);

  d3.select("svg").selectAll("circle")
    .attr("r", function(d){
      return calculateRadius(d, country);
    })
}
